...
/// Секреты: Anthropic:ApiKey (dotnet user-secrets set "Anthropic:ApiKey" "ваш_ключ").
/// Настройки: Anthropic:Endpoint, Anthropic:Model (appsettings.json).
/// </summary>
internal class HarnessClaudeProgram(IConfiguration config, ILogger<HarnessClaudeProgram> logger) : IHarness
{
    public async Task RunAsync(CancellationToken cancellationToken = default)
    {
        var endpoint = new Uri(config["Anthropic:Endpoint"] ?? "https://api.anthropic.com/v1");
        var apiKey = config["Anthropic:ApiKey"]
            ?? throw new InvalidOperationException("Anthropic:ApiKey не задан. Выполните: dotnet user-secrets set \"Anthropic:ApiKey\" \"<ваш_ключ>\"");
        var modelName = config["Anthropic:Model"] ?? "claude-opus-4-5";

        logger.LogInformation("Anthropic harness запущен. Endpoint={Endpoint} Model={Model}", endpoint, modelName);
